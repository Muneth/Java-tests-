...
import static com.atempo.TestTags.NUTANIX;
...
@Tag(LINUX_AGENT)
@Tag(NUTANIX)
class BackupSelectionAppNutanixLinuxTest extends AbstractBackupSelectionTest {
  ...
  String getBackupSelectionPath() throws Exception {
    ...
    return UtilRequestHttp.recupPathBackup(SERVER_URL, AgentsResourceTest.retrieveOrCreateAppNutanix(), getToken(),
                                           USER_LINUX, LINUX_USER_PASSWORD, data, NUTANIX_BACKUP_VM_NAME);
  }
  ...
  String getOtherBackupSelectionPath() throws Exception {
    String data = new JSONObject().put("isDirectoryOnly", false).put("presentationPathFull", "/Virtual Machines")
                                  .toString(); // path discovery using the web ui in the test auto, browse APP_AppNutanixLinux to create a backup selection

    return UtilRequestHttp.recupPathBackup(SERVER_URL, AgentsResourceTest.retrieveOrCreateAppNutanix(), getToken(),
                                           USER_LINUX, LINUX_USER_PASSWORD, data, NUTANIX_BACKUP_VM_NAME_OTHER);
  }

  @Override
  String getRetrieveOrCreateApp() {
    return AgentsResourceTest.retrieveOrCreateAppNutanix();
  }
